...
export class WarmPoolManager {
  ...
  private async create(pool: ResolvedWarmPool): Promise<void> {
    ...
    try {
      ...
      const expiresAt = new Date(now.getTime() + pool.maxWarmAgeMs + parseDurationMs(template.spec.timeouts.maxAge));
  ...
  async inventory(): Promise<WarmPoolInventory> {
    ...
    return {
      items: this.deps.pools.map((pool) => {
        ...
        const ready = matching.filter((row) => row.state === "ready" && row.readyAt).map((row) => row.readyAt as Date);
        return {
          ...
          counts,
          oldest_ready_age_ms: ready.length ? now.getTime() - Math.min(...ready.map((date) => date.getTime())) : null,
        };
      }),
      metrics: {
        ...this.metrics,
