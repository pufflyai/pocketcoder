...
describe("failure log summary", () => {
  test("drops partial UTF-8 and line prefixes and redacts credentials", () => {
    ...
    expect(decodeFailureLogTail(encoded.subarray(1), true)).toBe("[redacted]\nPermissionError: /home/onefin/.pi\n");
  });
});
...
describe("scheduler admission", () => {
  ...
  test("respects the global active limit and preserves FIFO", async () => {
    ...
    const rows = [];
    for (let i = 0; i < 4; i += 1) {
      rows.push(await queueWorkspace(store, principal.id, echoRow.id, echo, `task-${i}`, new Date(t0 + i)));
    }
    ...
    expect(states.map((s) => s?.state)).toEqual(["provisioning", "provisioning", "queued", "queued"]);
  });
  ...
  test("does not exceed active limits across scheduler coordinators", async () => {
    ...
    await Promise.all([makeScheduler(store, driver, limits).tick(), makeScheduler(store, driver, limits).tick()]);
  ...
  test("admits an eligible principal beyond a blocked global queue prefix", async () => {
    ...
    const eligible = await queueWorkspace(store, b.id, echoRow.id, echo, "eligible-0", new Date(now + 1_000));
  ...
  test("bounded launch retry, then failed", async () => {
    ...
    expect(new TextDecoder().decode(logs[0]?.content)).toContain("workspace launch failed: fake driver create failure");
  });
});
...
describe("scheduler sweeps", () => {
  ...
  test("registration timeout fails a provisioning workspace and cleans the provider", async () => {
    ...
    await store.updateWorkspace(ws.id, { registrationExpiresAt: new Date(Date.now() - 1000) }, new Date());
    await scheduler.sweep();
    const after = await store.getWorkspace(ws.id);
    expect(after?.state).toBe("failed");
