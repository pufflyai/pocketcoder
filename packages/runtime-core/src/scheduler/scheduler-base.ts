...
export class SchedulerContext {
  ...
  timeoutMs(row: WorkspaceRow, key: "start" | "maxAge" | "idle" | "disconnectGrace" | "terminateGrace"): number {
    return parseDurationMs(row.templateSnapshot.spec.timeouts[key]);
  }

  graceSeconds(row: WorkspaceRow): number {
    return Math.max(1, Math.ceil(this.timeoutMs(row, "terminateGrace") / 1000));
  }

  report(context: string, err: unknown): void {
    this.deps.onError?.(context, err);
  }

  async captureLaunchFailure(workspaceId: string, error: unknown, at: Date): Promise<WorkspacePatch> {
  ...
  async cleanupWorkspaceStorage(row: WorkspaceRow): Promise<void> {
    ...
    try {
      ...
      const at = this.now();
      await this.deps.store.updateWorkspaceStorage(storage.id, { state: "deleted", deletedAt: at }, at);
    } catch (error) {
      await this.deps.store.updateWorkspaceStorage(storage.id, { lastErrorCode: "storage_cleanup_failed" }, this.now());
      this.report(`storage.cleanup.${row.id}`, error);
    }
  }

  async finishRestoreOperation(
    row: WorkspaceRow,
