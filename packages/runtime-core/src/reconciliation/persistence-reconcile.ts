...
async function loadOperationContext(
  ...
  const checkpoint = operation.checkpointId ? await deps.store.getCheckpoint(operation.checkpointId) : null;
  ...
  let storage: WorkspaceStorageRow | null = null;
  if (workspace) storage = await deps.store.getWorkspaceStorage(workspace.id);
  else if (checkpoint) storage = await deps.store.getStorage(checkpoint.storageId);
...
async function failOperation(
  ...
  await deps.store.updateOperation(operation.id, { state: "failed", reasonCode, completedAt: now }, now);
}
...
async function failPreserve(
  ...
  if (storage) {
    await deps.store.updateWorkspaceStorage(storage.id, { state: "retained", lastErrorCode: "checkpoint_failed" }, now);
  }
  await cleanupProvider(deps, workspace);
  if (checkpoint.state === "creating") {
    await deps.store.updateCheckpoint(checkpoint.id, { state: "failed", reasonCode: "checkpoint_failed" }, now);
  }
...
async function reconcilePersistenceState(
  ...
  for (const physical of discoveredStorage) {
    if (!(await deps.store.getStorage(physical.storageId))) {
      deps.log?.(`reconcile: unknown storage ${physical.storageId}; quarantined for operator inspection`);
    }
  }
  for (const physical of discoveredCheckpoints) {
    if (!(await deps.store.getCheckpoint(physical.checkpointId))) {
      deps.log?.(`reconcile: unknown checkpoint ${physical.checkpointId}; quarantined for operator inspection`);
    }
  }
}
