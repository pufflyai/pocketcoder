...
function fixtureClient(respond: (request: Request) => Response | Promise<Response>, requests: Request[] = []) {
...
describe("attachments.upload", () => {
  test("PUTs raw bytes with disposition, media type, and length headers", async () => {
    ...
    const client = fixtureClient(() => Response.json(descriptor(attachmentId), { status: 201 }), requests);
    ...
    expect(new URL(request.url).pathname).toBe(`/v1/workspaces/${WORKSPACE}/attachments/${attachmentId}`);
