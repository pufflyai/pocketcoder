...
function fixtureClient(route: (request: Request) => Response | Promise<Response>, requests: Request[] = []) {
  const fetchImpl = (async (input: string | URL | Request, init?: RequestInit) => {
    const request = input instanceof Request ? new Request(input, init) : new Request(input.toString(), init);
    requests.push(request);
    return await route(request);
  }) as typeof fetch;
  return new PocketCoderClient({ baseUrl: "http://pocketcoder.test", apiKey: "pkt_example", maxRetries: 0 }, fetchImpl);
}
...
describe("WorkspaceTurnResolver", () => {
  ...
  test("waits for preservation, validates the resumed generation, and waits for ready", async () => {
    ...
    expect(requests.some((request) => request.url.endsWith(`/${RESUMED}/changes?after=1&wait=30`))).toBe(true);
  });
  ...
  test("caller abort stops only that wait after resume starts", async () => {
    ...
    let callbackCalls = 0;
    const client = fixtureClient((request) =>
      Response.json(new URL(request.url).pathname.endsWith(RESUMED) ? resumed() : workspace({ state: "preserved" })),
    );
    const resolver = new WorkspaceTurnResolver({
      client,
      resumeWorkspace: async () => {
