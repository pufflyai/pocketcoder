...
export function createPersistenceTables(
  ...
  const workspaceCheckpoints = table(
    "workspace_checkpoints",
    {
      ...
      fileCount: bigint("file_count", { mode: "number" }),
      conversationRestore: text("conversation_restore")
        .$type<NonNullable<WorkspaceCheckpointRow["conversationRestore"]>>()
        .notNull(),
      label: text("label"),
      createdAt: timestamptz("created_at").notNull(),
      updatedAt: timestamptz("updated_at").notNull(),
    ...
    (table) => [
      check("workspace_checkpoints_state_check", sql`${table.state} IN ${sqlValues(CHECKPOINT_STATES)}`),
      check(
      ...
      index("workspace_checkpoints_principal_state").on(table.principalId, table.state, table.createdAt),
      index("workspace_checkpoints_gc").on(table.state, table.expiresAt),
    ],
  );

  const workspaceOperations = table(
    "workspace_operations",
    {
      ...
      principalId: uuid("principal_id")
        .notNull()
        .references(() => principals.id),
      kind: text("kind").$type<NonNullable<WorkspaceOperationRow["kind"]>>().notNull(),
      state: text("state").$type<NonNullable<WorkspaceOperationRow["state"]>>().notNull(),
      ...
      reasonCode: text("reason_code").$type<NonNullable<WorkspaceOperationRow["reasonCode"]>>(),
    ...
    (table) => [
      ...
      check("workspace_operations_state_check", sql`${table.state} IN ${sqlValues(OPERATION_STATES)}`),
      unique().on(table.principalId, table.kind, table.idempotencyKey),
      index("workspace_operations_due").on(table.state, table.updatedAt),
    ],
  );

  return { workspaceStorage, workspaceCheckpoints, workspaceOperations };
}
