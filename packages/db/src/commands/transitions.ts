...
import { AdmissionCommands } from "./admission";
import { asDate, enumValue, nullableEnumValue, type Row } from "./base";

export class TransitionCommands extends AdmissionCommands {
  protected patchSql(patch: WorkspacePatch, params: unknown[]): string[] {
    const sets: string[] = [];
    for (const [key, column] of Object.entries(TransitionCommands.PATCH_COLUMNS)) {
      if (!(key in patch)) continue;
      const value = (patch as Record<string, unknown>)[key];
      if (TransitionCommands.JSONB_PATCH_KEYS.has(key)) {
  ...
  async updateWorkspace(id: string, patch: WorkspacePatch, at: Date): Promise<void> {
    ...
    const bumpsChange = Object.keys(patch).some((key) =>
      TransitionCommands.CHANGE_PATCH_KEYS.has(key),
    );
