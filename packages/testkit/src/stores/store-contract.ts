...
async function prepared(harness: StoreContractHarness): Promise<PreparedStore> {
  ...
  const principal = await instance.store.createPrincipal(`contract-${randomUUID()}`, ["admin"], [template.name]);
...
async function withStore(harness: StoreContractHarness, run: (fixture: PreparedStore) => Promise<void>): Promise<void> {
  ...
  describe.skipIf(harness.enabled === false)(`${name} store contract`, () => {
    test("reserves queue capacity and idempotency atomically", async () => {
      await withStore(harness, async ({ store, workspace }) => {
        ...
        expect(results.map((result) => result.kind).sort()).toEqual(["capacity_exceeded", "created"]);
    ...
    test("transitions state, history, notification, and outbox together", async () => {
      await withStore(harness, async ({ store, workspace }) => {
        ...
        expect((await store.listStateHistory(inserted.workspace.id)).map((row) => row.toState)).toEqual([
          "queued",
          "provisioning",
        ]);
        expect((await store.claimDueEvents(new Date(at.getTime() + 1), 10)).map((row) => row.eventType)).toEqual([
          "workspace.queued",
  ...
  describe.skipIf(harness.enabled === false)(`${name} persistence store contract`, () => {
    test("replays operations before capacity and preserves ready checkpoint immutability", async () => {
      await withStore(harness, async ({ store, principal, template, workspace }) => {
        ...
        expect((await store.insertOperation(operation, { maxIncompleteOperations: 1 })).created).toBe(true);
        expect((await store.insertOperation(operation, { maxIncompleteOperations: 1 })).created).toBe(false);
        ...
        await expect(store.updateCheckpoint(checkpointId, { providerKind: "changed" }, new Date())).rejects.toThrow(
          "immutable",
        );
  ...
  describe.skipIf(harness.enabled === false)(`${name} terminal store contract`, () => {
    test("audits sessions and enforces the open-session limit", async () => {
      await withStore(harness, async ({ store, machineKey, workspace }) => {
        ...
        expect(await store.listTerminalSessions(inserted.workspace.id, undefined, 10)).toEqual([closed]);
      });
    });
  });
}
