...
export class ProxyStreamCoordinator {
  ...
  async relay(request: ProxyRequest, service: TemplateService, route: TemplateServiceRoute): Promise<void> {
  ...
  private async forwardBody(stream: ActiveStream, reader: BodyReader, maxResponseBytes: number): Promise<boolean> {
    let seq = 0;
    let totalBytes = 0;
    while (true) {
