...
export class AttachmentManager {
  ...
  private async chunk(payload: AttachmentChunk): Promise<void> {
    ...
    if (upload.received + bytes.byteLength > upload.declared) {
      await this.failAndDiscard(payload.operation_id, "too_large", "bytes exceed the declared length");
      return;
    }
  ...
  private async validated(attachmentId: string, sidecar: Sidecar): Promise<AttachmentDescriptor | null> {
  ...
  private async failAndDiscard(operationId: string, code: FailureCode, detail: string): Promise<void> {
