...
test("native checkpoint synchronizes stable messages before gracefully stopping AgentAPI", async () => {
  ...
  await writeFile(harnessPath, 'process.once("SIGTERM", () => process.exit(0)); setInterval(() => {}, 1000);\n');
  await writeFile(
    inputPath,
    JSON.stringify({
      workspace_id: workspaceId,
  ...
  try {
    await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(() => reject(new Error(`native checkpoint did not quiesce: ${JSON.stringify(frames)}`)), 5000),
      ),
    ]);
