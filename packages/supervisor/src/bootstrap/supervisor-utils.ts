...
export async function verifyWritableMemoryPaths(paths: string[]): Promise<void> {
  for (const path of paths) {
    ...
    try {
      ...
      throw new Error(
        `writable memory preflight failed for ${path}: ${error instanceof Error ? error.message : "unknown error"}`,
        { cause: error },
      );
    } finally {
...
export function enforcedEnvironment(exec: ExecSpec, overrides: Record<string, string | undefined> = {}) {
