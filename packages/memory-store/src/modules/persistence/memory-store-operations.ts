...
export class MemoryOperationStore {
  constructor(private readonly context: Pick<MemoryState, "operations" | "checkpoints" | "workspaces" | "outputs">) {}
  async insertOperation(
    ...
    const existing = [...this.context.operations.values()].find(
      (candidate) =>
    ...
    if (
      ...
      [...this.context.operations.values()].filter((operation) => ["pending", "running"].includes(operation.state))
  ...
  assertOperationReferences(row: WorkspaceOperationRow): void {
    if (row.checkpointId && !this.context.checkpoints.has(row.checkpointId)) {
      throw new Error("operation checkpoint does not exist");
    }
    if (row.resultWorkspaceId && !this.context.workspaces.has(row.resultWorkspaceId)) {
  ...
  async getOperationByIdempotency(
    ...
    const row = [...this.context.operations.values()].find(
      (candidate) =>
        candidate.principalId === principalId && candidate.kind === kind && candidate.idempotencyKey === idempotencyKey,
    );
    return row ? { ...row } : null;
  }

  async listIncompleteOperations(): Promise<WorkspaceOperationRow[]> {
    return [...this.context.operations.values()]
  ...
  async checkpointUsage(principalId: string | null) {
    const rows = [...this.context.checkpoints.values()].filter(
      (row) => (!principalId || row.principalId === principalId) && (row.state === "ready" || row.state === "deleting"),
    );
    return {
      count: rows.length,
  ...
  async countIncompleteOperations(): Promise<number> {
    return [...this.context.operations.values()].filter((row) => row.state === "pending" || row.state === "running")
      .length;
  }
  ...
  async appendOutput(input: WorkspaceOutputRow): Promise<WorkspaceOutputRow> {
    ...
    list.push(row);
    this.context.outputs.set(input.workspaceId, list);
  ...
  async listOutputs(workspaceId: string): Promise<WorkspaceOutputRow[]> {
    return (this.context.outputs.get(workspaceId) ?? []).map((row) => ({ ...row }));
  }
}
