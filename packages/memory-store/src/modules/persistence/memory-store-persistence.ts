...
export class MemoryPersistenceStore {
  constructor(private readonly context: Pick<MemoryState, "storage" | "checkpoints">) {}
  async insertWorkspaceStorage(row: WorkspaceStorageRow): Promise<WorkspaceStorageRow> {
    const existing = [...this.context.storage.values()].find(
      (candidate) =>
        candidate.workspaceId === row.workspaceId && !["deleted", "lost", "quarantined"].includes(candidate.state),
    );
    if (existing) return { ...existing };
    this.context.storage.set(row.id, { ...row });
    return { ...row };
  }

  async getWorkspaceStorage(workspaceId: string): Promise<WorkspaceStorageRow | null> {
    const row = [...this.context.storage.values()].find(
      (candidate) =>
        candidate.workspaceId === workspaceId && !["deleted", "lost", "quarantined"].includes(candidate.state),
    );
    return row ? { ...row } : null;
  }
  ...
  async insertCheckpoint(row: WorkspaceCheckpointRow): Promise<WorkspaceCheckpointRow> {
    const existing = this.context.checkpoints.get(row.id);
    if (existing) return { ...existing };
    this.context.checkpoints.set(row.id, { ...row });
    return { ...row };
  }
  ...
  async listCheckpoints(
    ...
    return [...this.context.checkpoints.values()]
      .filter(
        (row) =>
