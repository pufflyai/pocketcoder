...
export class MemoryWorkspaceStore {
  constructor(private readonly context: Pick<MemoryState, "workspaces" | "appendHistory" | "appendWorkspaceEvent">) {}
  workspaceInsertConflict(row: WorkspaceInsert): WorkspaceInsertResult | null {
    for (const existing of this.context.workspaces.values()) {
  ...
  queueIsFull(maxQueuedWorkspaces: number | undefined): boolean {
    return (
      maxQueuedWorkspaces !== undefined &&
      [...this.context.workspaces.values()].filter((workspace) => workspace.state === "queued").length >=
        maxQueuedWorkspaces
    );
  }
  ...
  async insertWorkspace(
    ...
    this.context.workspaces.set(workspace.id, workspace);
    this.context.appendHistory(workspace, null, "queued", null, row.createdAt);
    this.context.appendWorkspaceEvent(workspace, row.createdAt);
    return { kind: "created", workspace: { ...workspace } };
  }

  async getWorkspaceByIdempotency(principalId: string, idempotencyKey: string): Promise<WorkspaceRow | null> {
    const workspace = [...this.context.workspaces.values()].find(
      (candidate) => candidate.principalId === principalId && candidate.idempotencyKey === idempotencyKey,
    );
  ...
  async listWorkspaces(principalId: string, filter: WorkspaceListFilter): Promise<WorkspaceRow[]> {
    let rows = [...this.context.workspaces.values()].filter((w) => w.principalId === principalId);
