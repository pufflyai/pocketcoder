...
describe("filesystem checkpoint storage", () => {
  test("snapshots, verifies, and restores independent writable forks", async () => {
    ...
    await expect(driver.verifyCheckpoint(snapshot.ref, snapshot.manifest)).resolves.toEqual(snapshot.manifest);
    ...
    expect((await lstat(join(String(fork.ref.root), "worktree", "current.ts"))).isSymbolicLink()).toBe(true);
    await writeFile(restored, "fork-only\n");
    await expect(driver.verifyCheckpoint(snapshot.ref, snapshot.manifest)).resolves.toEqual(snapshot.manifest);
    expect(await readFile(join(String(snapshot.ref.root), "worktree", "src", "index.ts"), "utf8")).toBe(
    ...
    await expect(driver.verifyCheckpoint(snapshot.ref, snapshot.manifest)).rejects.toThrow("content digest mismatch");
  ...
  test("rejects escaping symlinks and measured quota overflow", async () => {
    ...
    await expect(
      driver.snapshot(allocation.ref, randomUUID(), "sha256:template", [{ ...worktreeMount, maxBytes: 4 }]),
    ).rejects.toThrow("maxBytes");
    await rm(join(worktree, "large"));
    await symlink("../../etc/passwd", join(worktree, "escape"));
    await expect(driver.snapshot(allocation.ref, randomUUID(), "sha256:template", mounts)).rejects.toThrow("symlink");
    await expect(
