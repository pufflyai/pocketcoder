...
describe("Kubernetes workspace driver", () => {
  ...
  test("projects the portable launch contract into a namespaced Job", async () => {
    ...
    expect(job.spec.template.spec.containers[0]?.env).toEqual([{ name: "SAFE_VALUE", value: "yes" }]);
    expect(job.spec.template.spec.containers[0]?.volumeMounts.map((mount) => mount.mountPath)).toContain("/workspace");
    expect(job.spec.template.spec.containers[0]?.volumeMounts.map((mount) => mount.mountPath)).toContain(
      "/run/pocketcoder/secrets/model-key",
    );
    expect(job.spec.template.spec.containers[0]?.volumeMounts.map((mount) => mount.mountPath)).toContain(
      "/home/onefin",
    );
  ...
  test("places NET_ADMIN and audit credentials only in a restartable native sidecar", async () => {
    ...
    expect(JSON.parse(providerSecret.stringData["input.json"] as string).server_url).toBe("http://127.0.0.1:18081");
    ...
    expect(workspaceContainer?.volumeMounts.map((mount) => mount.name)).not.toContain("egress-config");
  });
});
