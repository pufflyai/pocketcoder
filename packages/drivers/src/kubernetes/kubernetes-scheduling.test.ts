...
import { resourceRequirements, schedulingFields, validateToleration } from "./kubernetes-scheduling";
...
describe("Kubernetes toleration validation", () => {
  ...
  test("rejects a value with the Exists operator", () => {
    expect(() => validateToleration({ operator: "Exists", value: "workspace", effect: "NoSchedule" })).toThrow(
      "operator Exists must not set a value",
    );
  });
  ...
  test("rejects other effects", () => {
    expect(() => validateToleration({ effect: "NoExecute" } as never)).toThrow("toleration effect must be NoSchedule");
  });
});
