...
async function deleteAndWait(kind: "job" | "pod", name: string) {
  await kubectl(["-n", namespace(), "delete", kind, name, "--ignore-not-found", "--wait=true", "--timeout=30s"]);
}
...
const nfsEnabled =
  process.env.POCKETCODER_KUBERNETES_CONFORMANCE === "1" && Boolean(process.env.POCKETCODER_KUBERNETES_WORKSPACE_CLAIM);
...
function nfsSetupJob(name: string, claimName: string, own: string, sibling: string, checkpoint: string) {
  return {
    apiVersion: "batch/v1",
    kind: "Job",
    metadata: { name, namespace: namespace() },
...
function nfsCleanupPod(name: string, claimName: string, own: string, sibling: string, checkpoint: string) {
  return {
    apiVersion: "v1",
    kind: "Pod",
    metadata: { name, namespace: namespace() },
...
async function clusterEvidence(setupLogs: string, probeLogs: string) {
  ...
  if (!region || !nfsTier) {
    throw new Error("POCKETCODER_DIGITALOCEAN_REGION and POCKETCODER_DIGITALOCEAN_NFS_TIER are required");
  }
