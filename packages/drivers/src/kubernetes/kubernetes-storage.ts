...
import { FilesystemStorageDriver, type FilesystemStorageDriverOptions } from "../filesystem/filesystem-storage";
...
export class KubernetesPvcStorageDriver implements WorkspaceStorageDriver {
  ...
  async cloneCheckpoint(checkpoint: CheckpointRef, target: StorageRef, manifest: CheckpointManifest): Promise<void> {
  ...
  async verifyCheckpoint(ref: CheckpointRef, expected: CheckpointManifest): Promise<CheckpointManifest> {
