...
import type { RuntimeSecretRef, WorkspaceRow, WorkspaceSecretResolver } from "@pstdio/pocketcoder-runtime-core";
...
export class KubernetesSecretResolver implements WorkspaceSecretResolver {
  ...
  async resolve(workspace: WorkspaceRow): Promise<RuntimeSecretRef[]> {
    return [...collect(workspace)].map((reference) => {
      ...
      const volumeName =
        `${secretName}-${key.replace(/[^a-z0-9-]/gi, "-")}`.toLowerCase().slice(0, 63).replace(/-+$/g, "") || "secret";
      return {
        name: volumeName,
  ...
  async resolveSourceCredential(workspace: WorkspaceRow): Promise<string | null> {
    ...
    const reference =
      workspace.templateSnapshot.spec.source?.repositories[workspace.sourceDescriptor.repository]?.credential;
