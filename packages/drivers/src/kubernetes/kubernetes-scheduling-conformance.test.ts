...
function schedulingConfig() {
  return {
    nodeSelector: JSON.parse(process.env.POCKETCODER_KUBERNETES_NODE_SELECTOR as string) as Record<string, string>,
    tolerations: JSON.parse(process.env.POCKETCODER_KUBERNETES_TOLERATIONS as string) as KubernetesToleration[],
  };
}
...
async function podForJob(name: string) {
  const output = await kubectl(["-n", namespace(), "get", "pods", "-l", `job-name=${name}`, "-o", "json"]);
...
async function waitForUnschedulable(name: string) {
  ...
  while (Date.now() < deadline) {
    ...
    if (!pod?.spec?.nodeName && condition?.status === "False" && condition.reason === "Unschedulable") {
      return;
    }
...
describe.skipIf(!enabled)("Kubernetes workspace scheduling conformance", () => {
  test("requires the taint toleration and preserves ephemeral storage", async () => {
    ...
    try {
      ...
      await kubectl(["-n", namespace(), "wait", `job/${scheduledName}`, "--for=condition=complete", "--timeout=120s"]);
