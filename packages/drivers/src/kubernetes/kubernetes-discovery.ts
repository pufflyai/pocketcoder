...
export function discoveredWorkspaces(output: string, kind: string, namespace: string): DiscoveredProvider[] {
...
export function discoveredWarmRuntimes(output: string, kind: string, namespace: string): DiscoveredWarmProvider[] {
  const list = JSON.parse(output) as JobList;
  return (list.items ?? []).flatMap((job) => {
