...
export function appendWorkspaceMounts(args: string[], launch: WorkspaceLaunch): void {
  for (const mount of launch.mounts) {
    ...
    args.push("--mount", `type=bind,src=${mount.source.path},dst=${mount.target}${mount.readOnly ? ",readonly" : ""}`);
  }
  for (const secret of launch.secrets) {
    if (secret.source.kind !== "host-path") {
      throw new Error(`docker driver cannot consume ${secret.source.kind} secrets; configure a file secret resolver`);
    }
