...
import { templateServices } from "../templates/template-runtime";
import { ServiceRouteSchema } from "../templates/template-schema";
import {
  AgentFrameSchema,
  PROTOCOL_VERSION,
...
describe("streamed relay contracts", () => {
  test("defaults existing routes to buffered and declares AgentAPI events as streamed", () => {
    expect(ServiceRouteSchema.parse({ method: "GET", path: "/status" }).responseMode).toBe("buffered");
    ...
    const eventRoute = templateServices(native.manifest.spec).agent?.routes.find((route) => route.path === "/events");
