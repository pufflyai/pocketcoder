...
describe("AgentAPI transport", () => {
  ...
  test("rejects AgentAPI state persistence for ACP transport", () => {
    ...
    expect(() => parseTemplateManifest(stateFile)).toThrow("stateFile is only valid for PTY transport");
    ...
    expect(() => parseTemplateManifest(restore)).toThrow("supported conversation restore requires PTY transport");
  });

  test("rejects an unnormalized AgentAPI state file", () => {
    const manifest = nativeManifest();
    (manifest.spec as { agent: Record<string, unknown> }).agent.stateFile = "/state/../tmp/agentapi.json";

    expect(() => parseTemplateManifest(manifest)).toThrow("stateFile must be a normalized absolute filesystem path");
  });
