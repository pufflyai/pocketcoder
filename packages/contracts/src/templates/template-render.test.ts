...
describe("template rendering", () => {
  ...
  test("rejects placeholders, bad pointers, untyped values, and invalid final manifests", () => {
    ...
    expect(() => renderTemplateManifest(manifest(), { image: "registry.test/agent:latest" })).toThrow();
    expect(() => renderTemplateManifest(manifest(), { image: IMAGE, set: ["/spec/missing=true"] })).toThrow(
      "does not exist",
    );
    expect(() => renderTemplateManifest(manifest(), { image: IMAGE, set: ["/spec/resources/cpu=not-json"] })).toThrow(
      "valid JSON",
    );
    expect(() => renderTemplateManifest(manifest(), { image: IMAGE, set: ["/spec/resources/cpu=false"] })).toThrow();
  });

  test("requires a plain release triplet before automatic versioning", () => {
    const input = manifest();
    input.spec.version = "1.2.3-existing";
    expect(() => renderTemplateManifest(input, { image: IMAGE })).toThrow("spec.version must be a release triplet");
  });
});
