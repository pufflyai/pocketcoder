...
import type { PersistenceMount } from "../persistence/persistence";
...
function envSources(spec: TemplateSpec): Array<[Array<string | number>, Record<string, string>]> {
  const sources: Array<[Array<string | number>, Record<string, string>]> = [
    [["spec", "env"], spec.env],
    [isAgentApiNative(spec) ? ["spec", "agent", "env"] : ["spec", "harness", "env"], agentApiHarness(spec).env],
  ];
...
function validateServiceRoutes(serviceName: string, routes: TemplateServiceRoute[], ctx: z.RefinementCtx): void {
...
function validateEnvironment(spec: TemplateSpec, ctx: z.RefinementCtx): void {
  for (const [where, env] of envSources(spec)) {
    for (const [key, value] of Object.entries(env)) {
      const literalSecret = SECRET_ENV_PATTERN.test(key) && !value.startsWith(SECRET_REFERENCE_PREFIX) && value !== "";
      if (literalSecret) {
        ctx.addIssue({
          code: "custom",
          path: where,
...
const FORBIDDEN_PERSISTENCE_ROOTS = ["/", "/run/pocketcoder", "/run/pocketcoder/secrets", "/proc", "/sys", "/dev"];
...
function validateAgentStateFile(spec: TemplateSpec, mounts: PersistenceMount[], ctx: z.RefinementCtx): void {
...
function validateSourceMount(spec: TemplateSpec, mounts: PersistenceMount[], ctx: z.RefinementCtx): void {
