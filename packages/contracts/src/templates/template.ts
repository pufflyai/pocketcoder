import { z } from "zod";
import { canonicalJson, digestOf } from "../common/canonical";
...
export function normalizeTemplateSnapshot(snapshot: TemplateSnapshot): TemplateSnapshot {
  return {
    ...
    ...(snapshot.services ? { services: z.record(z.string(), ServiceSchema).parse(snapshot.services) } : {}),
  };
}
...
export function findRoute(
  ...
  const route = foundService.routes.find((candidate) => candidate.method === method && candidate.path === path);
