...
import { AgentFrameSchema, ClientTerminalMessageSchema, ServerFrameSchema, TerminalSessionPageSchema } from "../index";
...
describe("terminal contracts", () => {
  ...
  test("rejects oversized chunks and invalid dimensions", () => {
    ...
    expect(ClientTerminalMessageSchema.safeParse({ type: "input", data_b64: oversized }).success).toBe(false);
    expect(ClientTerminalMessageSchema.safeParse({ type: "resize", rows: 0, cols: 80 }).success).toBe(false);
  });
