...
import { z } from "zod";
import { CursorPageSchema, CursorQuerySchema } from "../common/pagination";
...
export function findNetworkRule(policy: NetworkPolicy, host: string, port: number): NetworkRule | null {
  if (policy.mode !== "restricted") return null;
  const normalized = host.toLowerCase();
  return policy.allow.find((rule) => rule.ports.includes(port) && matchesDomain(rule.domain, normalized)) ?? null;
}
...
export const NetworkEventInputSchema = z.object({
  ...
  path: z
    .string()
    .max(2048)
    .refine((path) => !path.includes("?") && !path.includes("#"), "path cannot contain query or fragment")
    .nullable()
    .default(null),
