...
import { findNetworkRule, isPrivateAddress, NetworkEventBatchSchema, NetworkPolicySchema } from "./network";
...
describe("network policy", () => {
  test("normalizes unrestricted and restricted policies", () => {
    ...
    expect(NetworkPolicySchema.parse({ mode: "restricted", allow: [{ domain: "github.com" }] })).toEqual({
      mode: "restricted",
  ...
  test("rejects ambiguous domains and ports", () => {
    for (const domain of ["Example.com", "127.0.0.1", "*example.com", "api.*.com", "example.com."]) {
      expect(() => NetworkPolicySchema.parse({ mode: "restricted", allow: [{ domain }] })).toThrow();
    }
    expect(() =>
