...
import type { CommandContext, CommandRegistrar } from "../session/commands";
import { TargetRef } from "../session/session-target";
import { formatConversationMessage, type ThemeLike } from "../ui/renderers";
import { collectTurnFiles, DIRECT_MODE_ATTACHMENT_ERROR, pathTokens, registerAttachCommand } from "./attachments";
...
describe("RemoteAgentClient with attachments", () => {
  test("includes attachment_ids in the AgentAPI message body", async () => {
    ...
    const message = requests.find((request) => request.method === "POST" && request.url.endsWith("/message"));
