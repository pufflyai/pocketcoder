...
import { type WorkspaceResource, WorkspaceTurnResolutionError, WorkspaceTurnResolver } from "@pstdio/pocketcoder-sdk";
import { captureTurnAttachmentBatch } from "../attachments/attachments";
...
function attachmentResponse(
  ...
  if (state.options.failPhase === "attachment" && workspaceId && state.failures.delete(workspaceId)) {
    return terminal();
  }
...
function messagesResponse(workspaceId: string | undefined, targetId: string, state: HarnessState): Response {
...
function fetchHarness(options: HarnessOptions) {
  ...
  const controlPlane = new ControlPlaneClient({ baseUrl: "http://pocketcoder.test", key: "key" }, fetchImpl);
