...
export function extensionPath(moduleDir: string): string {
  // Source modules are nested; the published launcher and extension share dist.
...
export function resolvePiInvocation(options: ResolveOptions = {}): PiInvocation {
  ...
  const resolvePath = options.resolvePath ?? ((specifier: string) => fileURLToPath(import.meta.resolve(specifier)));
