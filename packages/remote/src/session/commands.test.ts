...
import { StatusPoller, statusText } from "../ui/status";
import { type CommandContext, type CommandUi, registerWorkspaceCommands, type WorkspaceCommandDeps } from "./commands";
...
function commandHarness(options: {
  ...
  const controlPlane = new ControlPlaneClient({ baseUrl: "http://pocketcoder.test", key: "pkt_example" }, fetchImpl);
  ...
  const registrar = {
    registerCommand(name: string, config: { handler: (args: string, ctx: CommandContext) => Promise<void> }) {
    ...
    const { recorded, targets, handlers, ctx } = commandHarness({
      router: (request) => {
        ...
        if (url.pathname === "/v1/templates") {
          return Response.json({
            items: [{ name: "pi-harness", version: "1", digest: "sha256:template", status: "active" }],
            next_cursor: null,
          });
        }
...
describe("status poller", () => {
  ...
  test("tracks workspace state and stops on a terminal transition", async () => {
    const { poller, statuses, notifications } = pollerHarness([{ state: "ready" }, { state: "canceled" }]);
