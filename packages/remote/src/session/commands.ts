...
import type { ControlPlaneClient, WorkspaceSummary } from "../client/control-plane";
import { WorkspaceTerminalError } from "../client/control-plane";
...
async function switchToWorkspace(deps: WorkspaceCommandDeps, ctx: CommandContext, workspaceId: string): Promise<void> {
...
async function createWorkspace(deps: WorkspaceCommandDeps, ctx: CommandContext): Promise<void> {
  ...
  if (!template) return;
  const externalId = (await ctx.ui.input("External id", `pi-${randomUUID()}`))?.trim() || `pi-${randomUUID()}`;
  ...
  try {
    ...
    const ready = await deps.controlPlane.workspaces.waitForReady(created, deps.waitTimeoutMs ?? 300_000, {
      onTick: (workspace) => ctx.ui.setWorkingMessage(`workspace ${workspace.state}`),
    });
    await switchToWorkspace(deps, ctx, ready.id);
  } catch (error) {
    const message =
      error instanceof WorkspaceTerminalError ? error.message : `workspace create failed: ${String(error)}`;
