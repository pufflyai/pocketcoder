...
import { TERMINAL_WORKSPACE_STATES } from "../client/control-plane";
...
export function statusText(workspace: Pick<WorkspaceSummary, "id" | "state" | "agent_state">): string {
...
export class StatusPoller {
  ...
  private async run(): Promise<void> {
    ...
    while (!this.stopped) {
      ...
      try {
        const change = await this.controlPlane.workspaces.change(this.workspaceId, this.cursor, this.waitSeconds, {
        ...
        this.ui.setStatus(STATUS_KEY, `${this.lastText ?? `ws ${this.workspaceId.slice(0, 8)}`} · reconnecting`);
