...
import { ControlPlaneClient, type ConversationMessage } from "../client/control-plane";
import {
...
function conversationClient(messages: ConversationMessage[], status = 200): ControlPlaneClient {
  const fetchImpl = (async (input: string | URL | Request, init?: RequestInit) => {
    ...
    if (status !== 200) {
      return Response.json({ error: { code: "conversation.expired", message: "gone", request_id: "r1" } }, { status });
    }
...
describe("conversation formatting", () => {
  ...
  test("renders notices with their level", () => {
    expect(formatHistoryNotice({ text: "expired", level: "warning" }, theme)).toBe("[warning]expired[/]");
