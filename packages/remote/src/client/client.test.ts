...
describe("local Pi AgentAPI client", () => {
  test("publishes full live snapshots before completing from stable messages", async () => {
      ...
      if (request.method === "GET" && path.endsWith("/messages")) {
        return Response.json({
          messages: [
            ...
            ...(finalMessage === "remote ready" ? [] : [{ id: 2, role: "agent", content: finalMessage }]),
          ],
        });
      }
    ...
    expect(await client.send("write", undefined, [], (snapshot) => snapshots.push(snapshot))).toBe("rewritten final");
  ...
  test("sends a turn and waits for the new remote agent response", async () => {
    ...
    expect(requests.some((request) => new URL(request.url).pathname.endsWith("/changes"))).toBe(true);
    expect(requests.some((request) => new URL(request.url).pathname.endsWith("/status"))).toBe(false);
  });
