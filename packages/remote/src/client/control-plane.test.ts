...
import { relayTarget, TargetRef, targetFromEnvironment } from "../session/session-target";
import {
  ControlPlaneClient,
  ControlPlaneError,
...
function fixtureClient(
  ...
  return new ControlPlaneClient({ baseUrl: "http://pocketcoder.test/", key: "pkt_example" }, fetchImpl);
}
...
describe("control plane client", () => {
  ...
  test("maps 410 conversation errors to ConversationGoneError", async () => {
    ...
    expect(client.conversations.list("ws", { limit: 200 })).rejects.toBeInstanceOf(ConversationGoneError);
  });

  test("surfaces error codes on other failures", async () => {
    const client = fixtureClient(() =>
      Response.json({ error: { code: "auth.missing_scope", message: "no scope", request_id: "r2" } }, { status: 403 }),
    );
  ...
  test("waitForReady follows the change cursor until ready", async () => {
    ...
    const seen: string[] = [];
    const ready = await client.workspaces.waitForReady(workspace({ state: "queued", change_cursor: 0 }), 5_000, {
