...
function requestHeaders(request: IncomingMessage) {
  return Object.fromEntries(Object.entries(request.headers).filter(([name]) => !HOP_HEADERS.has(name.toLowerCase())));
}
...
export async function startProxy(options: ProxyOptions) {
  let sequence = 0;
  const resolve =
    options.resolve ?? (async (host: string) => lookup(host, { all: true, verbatim: true }) as Promise<Address[]>);
  const record = (
    input: Omit<NetworkEventInput, "source_seq" | "occurred_at" | "method" | "path" | "matched_rule"> &
  ...
  server.on("clientError", (_error, socket) => {
    ...
    socket.end(!accepted ? "HTTP/1.1 503 Service Unavailable\r\n\r\n" : "HTTP/1.1 400 Bad Request\r\n\r\n");
  });
