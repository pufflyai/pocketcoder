...
conformanceTest(
  ...
  async () => {
    ...
    try {
      ...
      try {
        await waitFor(async () => {
          const result = await docker(["exec", gateway.stdout, "/usr/local/bin/pocketcoder-egress", "health"], true);
      ...
      const identity = await docker(["exec", gateway.stdout, "sh", "-c", "grep -E '^(Uid|CapEff):' /proc/1/status"]);
