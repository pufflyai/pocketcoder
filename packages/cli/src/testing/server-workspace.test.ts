...
describe("pcd workspace workflows", () => {
  ...
  function workspaceResource(id: string, state: "queued" | "ready" | "failed", changeCursor: number) {
    return {
      id,
  ...
  test("creates a workspace and waits for the durable ready change", async () => {
    ...
    const server = Bun.serve({
      ...
      fetch(request) {
        ...
        if (request.method === "GET" && url.pathname === `/v1/workspaces/${waitingWorkspaceId}/changes`) {
          changeReads += 1;
    ...
    try {
      const result = await runCli(
        ["workspaces", "create", "--template", "pi-harness", "--wait", "--json", "--wait-timeout-seconds", "2"],
        {
          env: {
