...
describe("pcd doctor", () => {
  test.each([0, 2])("doctor waits through %i running statuses before sending and cancels", async (runningReads) => {
    ...
    const server = Bun.serve({
      ...
      async fetch(request) {
        ...
        if (request.method === "GET" && url.pathname === `/v1/workspaces/${doctorWorkspaceId}/changes`) {
          agentReads += 1;
        ...
        if (request.method === "GET" && url.pathname === `/v1/workspaces/${doctorWorkspaceId}/agent/status`) {
          return Response.json({ status: agentState() });
        }
        if (request.method === "POST" && url.pathname === `/v1/workspaces/${doctorWorkspaceId}/agent/message`) {
        ...
        if (request.method === "GET" && url.pathname === `/v1/workspaces/${doctorWorkspaceId}/agent/messages`) {
          return Response.json({
            messages: [{ id: 1, role: "assistant", content: diagnosticPrompt }],
          });
        }
        if (request.method === "POST" && url.pathname === `/v1/workspaces/${doctorWorkspaceId}/cancel`) {
          canceled = true;
  ...
  test.each([
  ...
  ] as const)("doctor rejects a %s status-only harness and still cancels", async (status, error) => {
    let canceled = false;
    const server = Bun.serve({
      hostname: "127.0.0.1",
      port: 0,
      fetch(request) {
        ...
        if (request.method === "GET" && url.pathname === `/v1/workspaces/${statusOnlyWorkspaceId}`) {
          return Response.json(doctorWorkspace(statusOnlyWorkspaceId, "ready", status));
        }
        if (request.method === "GET" && url.pathname === `/v1/workspaces/${statusOnlyWorkspaceId}/changes`) {
          return Response.json({
            cursor: 1,
            changed: false,
        ...
        if (request.method === "GET" && url.pathname === `/v1/workspaces/${statusOnlyWorkspaceId}/agent/status`) {
          return Response.json({ status });
        }
        if (request.method === "POST" && url.pathname === `/v1/workspaces/${statusOnlyWorkspaceId}/cancel`) {
          canceled = true;
    ...
    try {
      const result = await runCli(["doctor", "--template", "status-only", "--turn-timeout-seconds", "1"], {
        env: {
          POCKETCODER_URL: server.url.origin,
          POCKETCODER_KEY: "doctor-key",
        },
...
describe("pcd environment", () => {
  test("loads .env from the nearest project directory", async () => {
    ...
    try {
      writeFileSync(join(directory, ".env"), `POCKETCODER_URL=${server.url.origin}\nPOCKETCODER_KEY=from-dotenv\n`);
  ...
  test("supports explicit work directories and environment files", async () => {
    ...
    try {
      ...
      const result = await runCli(["--workdir", directory, "--env-file", "staging.env", "workspaces", "list"], {
