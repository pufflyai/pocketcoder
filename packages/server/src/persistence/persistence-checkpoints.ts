...
import { ApiError, canonicalJson, digestOf, type ResolvedSource } from "@pstdio/pocketcoder-contracts";
...
export class PersistenceCheckpointService {
  constructor(private readonly context: PersistenceContext) {}
  async verify(principal: PrincipalRow, checkpointId: string, idempotencyKey: string): Promise<WorkspaceOperationRow> {
    ...
    if (!inserted.created) {
      this.context.throwFailedOperation(inserted.operation);
    ...
    try {
      ...
      await this.context.failOperation(inserted.operation.id, "checkpoint_corrupt");
  ...
  async sourceResolved(workspaceId: string, source: ResolvedSource): Promise<void> {
    const workspace = await this.context.deps.store.getWorkspace(workspaceId);
    if (
    ...
    if (workspace.resolvedSource && canonicalJson(workspace.resolvedSource) !== canonicalJson(source)) {
      throw new Error("resolved source is immutable");
    }
    await this.context.deps.store.updateWorkspace(workspaceId, { resolvedSource: source }, this.context.now());
  }
  ...
  async publishOutput(workspaceId: string, name: string, value: unknown): Promise<void> {
    ...
    if (declaration.type === "gitSha" && (typeof value !== "string" || !/^[0-9a-f]{40,64}$/.test(value))) {
    ...
    await this.context.deps.store.appendOutput({
      workspaceId,
      seq: 0,
      name,
      value,
