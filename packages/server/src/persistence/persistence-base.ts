...
export class PersistenceContext {
  ...
  async replayedOperation(
    ...
    const operation = await this.deps.store.getOperationByIdempotency(principalId, kind, idempotencyKey);
  ...
  throwFailedOperation(operation: WorkspaceOperationRow): void {
    ...
    const failures: Partial<Record<OperationKind, Record<string, [ErrorCode, string]>>> = {
      preserve: {
        ...
        checkpoint_storage_lost: ["workspace.persistence_not_enabled", "Workspace persistent storage is not ready."],
      },
      restore: {
        image_unavailable: ["restore.image_unavailable", "The exact checkpoint template snapshot is unavailable."],
  ...
  async failOperation(id: string, reasonCode: string): Promise<void> {
    const at = this.now();
    await this.deps.store.updateOperation(id, { state: "failed", reasonCode, completedAt: at }, at);
  }

  async insertOperation(row: WorkspaceOperationRow) {
    try {
      ...
      if (error instanceof OperationCapacityExceededError) {
        throw new ApiError("storage.capacity_exhausted", "Too many checkpoint operations are already running.");
      }
      throw error;
    }
  }

  async assertPreserveAdmission(principalId: string, declaredBytes: number): Promise<void> {
  ...
  async assertMeasuredCheckpoint(principalId: string, logicalBytes: number, fileCount: number): Promise<void> {
    const [global, principal] = await Promise.all([
      this.deps.store.checkpointUsage(null),
      this.deps.store.checkpointUsage(principalId),
    ]);
