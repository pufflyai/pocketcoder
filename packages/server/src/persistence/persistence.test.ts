...
describe("persistent workspace preservation", () => {
  test("preserves, verifies, restores, and keeps forks independent", async () => {
    ...
    await writeFile(join(sourceRoot, "worktree", "src", "state.ts"), "export const checkpointed = true;\n");
    ...
    await waitFor(async () => (await testServer.store.getWorkspace(created.id))?.state === "preserved");
    ...
    expect(
      (
        await testServer.store.listWorkspaces((await testServer.store.listPrincipals())[0]?.id ?? "", {
    ...
    expect(((await repeated.json()) as { checkpoint: { id: string } }).checkpoint.id).toBe(checkpoint.id);
    ...
    expect(((await repeatedRestore.json()) as { workspace: { id: string } }).workspace.id).toBe(restored.workspace.id);
    const changedInputReplay = await testServer.request(`/v1/checkpoints/${checkpoint.id}/restore`, {
      method: "POST",
      headers: { "idempotency-key": "restore-1" },
      body: JSON.stringify({
        ...restoreBody,
    ...
    const restoredFile = join(String(restoredStorage?.providerRef.root), "worktree", "src", "state.ts");
    expect(await readFile(restoredFile, "utf8")).toContain("checkpointed = true");
    await writeFile(restoredFile, "fork mutation\n");
    await testServer.storageDriver.verifyCheckpoint(checkpoint?.providerRef as never, checkpoint?.manifest as never);
...
describe("persistent workspace outputs", () => {
  test("persists only declared, bounded output metadata", async () => {
    ...
    await expect(testServer.persistence.publishOutput(workspace.id, "undeclared", "value")).rejects.toThrow(
      "not declared",
    );
    const outputs = await testServer.request(`/v1/workspaces/${workspace.id}/outputs`);
    expect(outputs.status).toBe(200);
    expect(((await outputs.json()) as { items: Array<{ name: string }> }).items[0]?.name).toBe("commit");
  });
});
