...
import type { TerminalBridgeClosed } from "../terminals/terminal-bridge";
import { Hub } from "./hub";
...
describe("terminal bridge", () => {
  test("forwards terminal traffic and reports audited byte counts on close", () => {
    ...
    expect(agentFrames.map((value) => JSON.parse(value).type)).toEqual(["terminal_open", "terminal_input"]);
    expect(clientFrames.map((value) => JSON.parse(value).type)).toEqual(["opened", "output", "closed"]);
