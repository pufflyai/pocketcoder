import type { AttachmentAck, AttachmentResolved, AttachmentResult } from "@pstdio/pocketcoder-contracts";
...
export class AttachmentRegistry<TConnection extends { attachments: Map<string, AttachmentChannel> }> {
  ...
  next(connection: TConnection, operationId: string, timeoutMs: number): Promise<AttachmentEvent | null> {
    const channel = connection.attachments.get(operationId);
