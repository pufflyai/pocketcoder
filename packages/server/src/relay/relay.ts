...
async function requestBody(c: Context<AppEnv>, method: RelayMethod, maxBytes: number): Promise<string | undefined> {
...
function validateRelayResponse(response: BufferedRelayResponse, maxBytes: number): Buffer {
  switch (response.error_code) {
    ...
    case "deadline":
      throw new ApiError("relay.deadline_exceeded", "The workspace service did not respond in time.");
    case "too_large":
...
async function streamedRelayResponse(
  ...
  void deps.store.updateWorkspace(workspaceId, { lastActivityAt: new Date() }, new Date()).catch(() => {});
...
export function relayHandler(
  ...
  options?: {
    ...
    transformBodyB64?: (c: Context<AppEnv>, bodyB64: string | undefined) => Promise<string | undefined>;
  },
) {
  return async (c: Context<AppEnv>): Promise<Response> => {
    ...
    if (!match) {
      throw new ApiError("relay.route_not_allowed", `${method} ${path} is not declared by this template.`);
    }
    ...
    if (match.route.responseMode === "stream") {
      return await streamedRelayResponse(deps, id, request, match.route.maxResponseBytes, c.req.raw.signal);
    }
