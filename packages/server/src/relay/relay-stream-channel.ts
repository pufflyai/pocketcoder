...
export class RelayStreamRegistry<TConnection extends StreamConnection> {
  ...
  push(connection: TConnection, payload: ProxyStreamChunk): void {
    ...
    if (bytes.byteLength > PROXY_STREAM_CHUNK_BYTES || channel.totalBytes + bytes.byteLength > channel.maxBytes) {
  ...
  cancel(connection: TConnection, requestId: string, reason: ProxyStreamCancel["reason"], errorBody = true): void {
