...
function conversationResume(
  capability: WorkspaceRow["persistenceCapability"],
): WorkspaceResource["persistence"]["conversation_resume"] {
  if (capability === "supported") return { status: "supported", reason: null };
  if (capability === "filesystem_only") return { status: "unsupported", reason: "filesystem_only" };
  return { status: "unknown", reason: "capability_unknown" };
}
...
export function toResource(row: WorkspaceRow): WorkspaceResource {
  return {
    ...
    persistence: {
      ...
      conversation_resume: conversationResume(row.persistenceCapability),
...
export class WorkspaceService {
  ...
  private async activeTemplate(principal: PrincipalRow, body: WorkspaceCreateRequest) {
    if (!templateAuthorized(principal, body.template.name)) {
      throw new ApiError("template.not_authorized", `Principal is not authorized for template ${body.template.name}.`);
    }
  ...
  private validateCreateInput(template: TemplateRow, body: WorkspaceCreateRequest): void {
    ...
    if (body.source && (!template.spec.source || !(body.source.repository in template.spec.source.repositories))) {
      throw new ApiError("source.not_allowed", "The selected repository alias is not declared by this template.");
    }
  }
  ...
  async create(
    ...
    if (replay) {
      if (replay.requestDigest !== requestDigest) {
        throw new ApiError("idempotency.conflict", "This Idempotency-Key was already used with a different request.");
      }
  ...
  async waitForChange(
    ...
    while (true) {
      ...
      if (this.activeWaiters >= 1000 || workspaceWaiters >= 100) {
        throw new ApiError("capacity.waiters_full", "The workspace change-wait capacity is full; retry later.");
      }
      this.activeWaiters += 1;
      this.waitersByWorkspace.set(id, workspaceWaiters + 1);
      try {
        await this.deps.store.waitForWorkspaceChange(id, after, Math.max(1, deadline - Date.now()), signal);
      } finally {
        this.activeWaiters -= 1;
