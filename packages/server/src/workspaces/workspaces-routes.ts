...
import { MAX_WORKSPACE_CHANGE_WAIT_SECONDS } from "../observability/server-timing";
...
function workspaceListFilterOf(query: z.infer<typeof WorkspaceListQuerySchema>): WorkspaceListFilter {
...
export function registerWorkspaceRoutes({
  ...
  app.openapi(
    createRoute({
      ...
      responses: {
        200: {
          description: "Current workspace resource, returned after a newer durable change or the bounded wait",
          content: {
            "application/json": {
              schema: z.object({
