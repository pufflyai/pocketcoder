...
describe("workspace lifecycle API", () => {
  test("get, list, and idempotent cancel", async () => {
    ...
    // at `canceled`.
    const cancel1 = await app.request(`/v1/workspaces/${ws.id}/cancel`, authed(token, { method: "POST" }));
    expect(cancel1.status).toBe(200);
    expect(["terminating", "canceled"]).toContain(((await cancel1.json()) as { state: string }).state);
    ...
    const cancel2 = await app.request(`/v1/workspaces/${ws.id}/cancel`, authed(token, { method: "POST" }));
...
describe("historical conversations", () => {
  test("filters the principal-scoped session index by exact metadata and time", async () => {
    ...
    const metadata = encodeURIComponent(JSON.stringify({ product: "onefin", tenant: "tenant-7", user: "user-1" }));
    const response = await server.app.request(`/v1/workspaces?metadata=${metadata}`, authed(server.token));
  ...
  test("reads a terminal transcript with stable pages, deduplicates, and deletes content", async () => {
    ...
    const page1 = await server.app.request(`/v1/workspaces/${workspace.id}/conversation?limit=1`, authed(server.token));
    ...
    expect(firstPage.items).toEqual([expect.objectContaining({ message_id: "m-1", content: "Fix the failing test" })]);
    ...
    const expired = await server.app.request(`/v1/workspaces/${workspace.id}/conversation`, authed(server.token));
    expect(expired.status).toBe(410);
    expect(((await expired.json()) as { error: { code: string } }).error.code).toBe("conversation.expired");
    ...
    const afterDelete = await server.app.request(`/v1/workspaces/${workspace.id}/conversation`, authed(server.token));
    expect(afterDelete.status).toBe(410);
    expect(((await afterDelete.json()) as { error: { code: string } }).error.code).toBe("conversation.deleted");
  });
});
