...
function resolveWarmPools(env: Environment): WarmPoolConfigEntry[] {
  ...
  return value.map((item, index) => {
    if (!item || typeof item !== "object") throw new Error(`POCKETCODER_WARM_POOLS[${index}] must be an object`);
    ...
    const duration = (key: string, fallback: string) => {
      ...
      if (typeof candidate !== "string") throw new Error(`POCKETCODER_WARM_POOLS[${index}].${key} must be a duration`);
      try {
...
function resolveAdmissionLimits(env: Environment): AdmissionLimits {
  return {
    ...DEFAULT_LIMITS,
    globalActiveWorkspaces: intEnv(env, "POCKETCODER_MAX_ACTIVE_WORKSPACES", DEFAULT_LIMITS.globalActiveWorkspaces),
    perPrincipalActiveWorkspaces: intEnv(
      env,
      "POCKETCODER_MAX_ACTIVE_PER_PRINCIPAL",
      DEFAULT_LIMITS.perPrincipalActiveWorkspaces,
    ),
    maxQueuedWorkspaces: intEnv(env, "POCKETCODER_MAX_QUEUED_WORKSPACES", DEFAULT_LIMITS.maxQueuedWorkspaces),
  };
}

function resolvePersistenceLimits(env: Environment): PersistenceLimits {
  return {
    maxRetainedBytes: bytesEnv(env, "POCKETCODER_MAX_RETAINED_BYTES", DEFAULT_PERSISTENCE_LIMITS.maxRetainedBytes),
    ...
    maxCheckpointFiles: intEnv(env, "POCKETCODER_MAX_CHECKPOINT_FILES", DEFAULT_PERSISTENCE_LIMITS.maxCheckpointFiles),
