...
describe("Kubernetes scheduling configuration", () => {
  ...
  test("rejects malformed JSON", () => {
    expect(() => resolveKubernetesScheduling({ POCKETCODER_KUBERNETES_NODE_SELECTOR: "{" })).toThrow(
      "POCKETCODER_KUBERNETES_NODE_SELECTOR must be valid JSON",
    );
  ...
  test("rejects the wrong top-level JSON shapes", () => {
    expect(() => resolveKubernetesScheduling({ POCKETCODER_KUBERNETES_NODE_SELECTOR: "[]" })).toThrow(
      "POCKETCODER_KUBERNETES_NODE_SELECTOR must be a JSON object",
    );
    expect(() => resolveKubernetesScheduling({ POCKETCODER_KUBERNETES_TOLERATIONS: "{}" })).toThrow(
  ...
  test("rejects invalid toleration field types", () => {
    ...
    expect(() =>
      resolveKubernetesScheduling({
        POCKETCODER_KUBERNETES_TOLERATIONS: JSON.stringify([{ operator: "Unknown", effect: "NoSchedule" }]),
      }),
