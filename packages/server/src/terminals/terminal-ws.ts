...
export function terminalConnectValidator(deps: TerminalWsDeps): MiddlewareHandler<AppEnv> {
  return async (c, next) => {
    ...
    c.set("terminalAuth" as never, { workspaceId, sessionId, reattach, rows: 24, cols: 80 } as never);
    await next();
  };
}
...
async function terminalCapability(deps: TerminalWsDeps, c: Context<AppEnv>) {
  ...
  if (connection.protocolVersion < TERMINAL_MIN_PROTOCOL_VERSION) {
    throw new ApiError("terminal.protocol_unsupported", "The workspace supervisor does not support terminal sessions.");
  }
  return { workspaceId, maxSessions: terminal.maxSessions };
}

async function terminalSession(deps: TerminalWsDeps, c: Context<AppEnv>, workspaceId: string, maxSessions: number) {
...
export function terminalWsEvents(deps: TerminalWsDeps) {
  return (c: { get: (key: string) => unknown }): WSEvents => {
    ...
    return {
      onOpen: (_event, ws) => {
        deps.hub.openTerminal(auth.workspaceId, auth.sessionId, ws, auth.reattach, auth.rows, auth.cols);
      },
...
function handleClientMessage(deps: TerminalWsDeps, auth: TerminalWsAuth, raw: unknown, ws: WSContext): void {
  let value: unknown;
