...
async function terminalWorkspace(server: TestServer, template: "fixture-echo" | "fixture-terminal") {
...
async function readyTerminalWorkspace(server: TestServer, template: "fixture-echo" | "fixture-terminal") {
...
describe("remote terminals", () => {
  test("holds scope, workspace-state, template, and protocol gates before upgrade", async () => {
    ...
    const undeclared = await server.app.request(`/v1/workspaces/${withoutTerminal}/terminal`, authed(server.token));
    expect(undeclared.status).toBe(422);
    expect(((await undeclared.json()) as { error: { code: string } }).error.code).toBe("terminal.not_declared");

    const oldProtocol = await readyTerminalWorkspace(server, "fixture-terminal");
    connectTerminalAgent(server, oldProtocol, 3);
    const unsupported = await server.app.request(`/v1/workspaces/${oldProtocol}/terminal`, authed(server.token));
  ...
  test("returns principal-owned terminal session audit pages", async () => {
    ...
    const response = await server.app.request(`/v1/workspaces/${workspaceId}/terminal-sessions`, authed(server.token));
  ...
  test("rejects malformed reattach ids and enforces the audited session limit", async () => {
    ...
    expect(((await limited.json()) as { error: { code: string } }).error.code).toBe("terminal.session_limit");
  });
  ...
  test("upgrades a terminal WebSocket and audits the live session", async () => {
    ...
    try {
      ...
      const socket = new WebSocket(`ws://127.0.0.1:${listener.port}/v1/workspaces/${workspaceId}/terminal`, {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        socket.onclose = (event) => reject(new Error(`terminal WebSocket closed (${event.code}): ${event.reason}`));
      ...
      expect(eventTypes).toEqual(expect.arrayContaining(["workspace.terminal_opened", "workspace.terminal_closed"]));
    } finally {
