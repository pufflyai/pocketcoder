...
import type { RelayDeps } from "../relay/relay";
...
function liveConnection(hub: Hub, workspaceId: string): LiveConnection {
  ...
  if (conn.protocolVersion < ATTACHMENTS_MIN_PROTOCOL_VERSION) {
    throw new ApiError("attachment.unsupported", "The connected workspace supervisor does not support attachments.");
  }
  return conn;
}
...
async function* rechunk(stream: ReadableStream<Uint8Array> | null, size: number): AsyncGenerator<Buffer> {
  if (!stream) return;
...
function resultError(result: AttachmentResult): ApiError {
  switch (result.status) {
    case "conflict":
      return new ApiError("attachment.conflict", "This attachment ID exists with different bytes or metadata.");
    case "failed":
      switch (result.failure_code) {
        ...
        case "invalid":
        case "sequence":
          return new ApiError("attachment.invalid", result.detail ?? "The supervisor rejected the upload.");
        default:
          return new ApiError("attachment.interrupted", "The upload did not complete inside the workspace.");
      }
    default:
...
function uploadHeaders(c: Context<AppEnv>): UploadHeaders {
  ...
  if (!name) {
    throw new ApiError("attachment.invalid", "A Content-Disposition header with a filename is required.");
  }
  ...
  if (declared > ATTACHMENT_MAX_FILE_BYTES) {
    throw new ApiError("attachment.too_large", `Attachments are limited to ${ATTACHMENT_MAX_FILE_BYTES} bytes.`);
  }
...
async function streamUpload(
  ...
  if (sent !== headers.declared) {
    throw new ApiError("attachment.invalid", "The request body is shorter than its Content-Length.");
  }
...
function abandonUpload(hub: Hub, workspaceId: string, conn: LiveConnection, operationId: string, error: unknown): void {
...
async function resolveDescriptors(
  ...
  try {
    ...
    if (event?.kind !== "resolved") {
      throw new ApiError("attachment.interrupted", "The workspace did not resolve the attachments in time.");
    }
