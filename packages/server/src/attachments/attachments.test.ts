...
describe("attachment upload", () => {
  test("streams chunked frames through the supervisor and returns the descriptor", async () => {
    ...
    expect(types).toEqual(["attachment_start", "attachment_chunk", "attachment_chunk", "attachment_finish"]);
  });
  ...
  test("byte-identical retries return 200 and different bytes conflict with 409", async () => {
    ...
    expect(((await conflicting.json()) as { error: { code: string } }).error.code).toBe("attachment.conflict");
  });
  ...
  test("requires the attachments:write scope", async () => {
    ...
    expect(((await res.json()) as { error: { code: string } }).error.code).toBe("auth.missing_scope");
  });
  ...
  test("rejects invalid ids, missing filenames, and oversized declarations", async () => {
    ...
    expect(((await badId.json()) as { error: { code: string } }).error.code).toBe("attachment.invalid");
    ...
    expect(((await huge.json()) as { error: { code: string } }).error.code).toBe("attachment.too_large");
  });
  ...
  test("rejects uploads while the workspace is not ready, ended, or disconnected", async () => {
    ...
    await server.app.request(`/v1/workspaces/${ws.id}/cancel`, authed(server.token, { method: "POST" }));
  ...
  test("rejects uploads to supervisors that predate protocol v3", async () => {
    ...
    expect(((await res.json()) as { error: { code: string } }).error.code).toBe("attachment.unsupported");
  });
});
...
describe("agent message attachments", () => {
  ...
  for (const alias of ["agent", "services/agent"]) {
    test(`appends the manifest and strips ids through /${alias}/message`, async () => {
      ...
      expect(body.content).toContain(`/home/pocketcoder/.pcd/attachments/${attachmentId}/notes.txt`);
    });
  }
  ...
  test("rejects unknown attachment ids without sending anything to the agent", async () => {
    ...
    expect(((await res.json()) as { error: { code: string } }).error.code).toBe("attachment.not_found");
  ...
  test("rejects messages whose resolved attachments exceed the aggregate limit", async () => {
    ...
    expect(((await res.json()) as { error: { code: string } }).error.code).toBe("attachment.too_large");
    expect(agent.proxied).toEqual([]);
  });
  ...
  test("rejects duplicate ids as invalid", async () => {
    ...
    expect(((await res.json()) as { error: { code: string } }).error.code).toBe("attachment.invalid");
  });
